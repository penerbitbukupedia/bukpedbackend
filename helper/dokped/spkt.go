...
func GenerateSPKTDaerahCetak(project model.Project, strkey string) (filecontent []byte, err error) {
	if len(project.Members) == 0 {
		err = errors.New("penulis belum di set pada project ini")
		return
	}
...
www.bukupedia.co.id`, "", "L", false)
	...
	pdf.Ln(6)
	pdf.CellFormat(35, 6, "", "", 0, "L", false, 0, "") // Indent to align with "Permohonan"
	pdf.CellFormat(100, 6, "Buku", "", 1, "L", false, 0, "")
	pdf.Ln(10) // Line break for spacing
	...
	// Add the ISBN application request
	pdf.MultiCell(0, 6, "Menyerahkan hasil terbitan dari penerbit kami. Adapun data buku yang kami serahkan kepada Perpustakaan Daerah Jawa Barat adalah,", "", "L", false)
	pdf.Cell(40, 6, "Judul")
	pdf.Cell(5, 6, ":")
	pdf.MultiCell(0, 6, project.Title, "", "L", false)
	pdf.Ln(2)
	...
	// Add closing remarks
	pdf.MultiCell(0, 6, `Bersama dengan surat ini, kami sertakan buku yang sudah kami terbitkan. Kami harapkan juga balasannya pada email resmi kami penerbit@bukupedia.co.id jika sudah diterima dengan baik.
Demikian surat pengantar ini kami buat, atas perhatian dan kerjasamanya diucapkan terima kasih.`, "", "L", false)
...
func GenerateSPKTPusatCetak(project model.Project, strkey string) (filecontent []byte, err error) {
	if len(project.Members) == 0 {
		err = errors.New("penulis belum di set pada project ini")
		return
	}
...
www.bukupedia.co.id`, "", "L", false)
	...
	// Add recipient details
	pdf.MultiCell(0, 6, `Yth. Kelompok Penerimaan SSKCKR
	Sub Direktorat Deposit
	Direktorat Deposit Bahan Pustaka
	Perpustakaan Nasional Indonesia`, "", "L", false)
	pdf.Ln(10)

	// Add body content
	pdf.MultiCell(0, 6, "Berdasarkan Undang Undang Republik Indonesia Nomor 13 tahun 2018 Tentang Serah Simpan Karya Cetak dan Karya Rekam Pasal 4 dan 5. Bahwa setiap penerbit buku menyerahkan buku yang telah diterbitkan kepada Perpustakaan nasional. Bersama ini kami atas nama,", "", "L", false)
	pdf.Cell(40, 6, "Penerbit")
	pdf.Cell(5, 6, ":")
	pdf.Cell(100, 6, "PT. Penerbit Buku Pedia")
	pdf.Ln(6)
	...
	// Add the ISBN application request
	pdf.MultiCell(0, 6, "Menyerahkan hasil terbitan dari penerbit kami. Adapun data buku yang kami serahkan kepada Perpustakaan Nasional adalah,", "", "L", false)
	pdf.Cell(40, 6, "Judul")
	pdf.Cell(5, 6, ":")
	pdf.MultiCell(0, 6, project.Title, "", "L", false)
	pdf.Ln(2)
